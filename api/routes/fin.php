    ...
    try {
        ...
        $listSql = "
          ...
          SELECT
            ...
            (SELECT user_name FROM dados_dachser.t_voucher_logs WHERE voucher_id = v.id AND acao IN ('ENVIADO_OPERACAO','APROVADO_FISCAL','APROVADO_SUPERVISOR','REENVIO_APOS_AJUSTE','APROVADO_URGENTE','MASTER_APROVADO_OPERACAO','VOUCHER_ENVIADO','RASCUNHO_ENVIADO','VOUCHER_CRIADO','VOUCHER_CRIADO_LOTE','IMPORTADO_RM','MASTER_CRIADO','VOUCHER_CRIADO_BATCH') ORDER BY data_hora DESC LIMIT 1) AS enviado_por_user_name
            FROM page_v v
            LEFT JOIN (
...
$router->get('fin/vouchers/search', function ($params) {
    try {
        ...
        $vouchers = [];
        if ($exactNd) {
            $vouchers = finQuery("SELECT v.*, COALESCE(v.data_emissao_documento, dfv.data_emissao) AS data_emissao_documento, dfv.id_rm as dfv_id_rm, dfv.numero_processo as dfv_numero_processo, dfv.razao_social as dfv_razao_social, dfv.nome_beneficiario as dfv_nome_beneficiario, dfv.valor_nf as dfv_valor_nf, dfv.ref_fornecedor as dfv_ref_fornecedor, dfv.mawb_mbl as dfv_mawb_mbl, COALESCE(NULLIF(v.ref_fornecedor, ''), dfv.ref_fornecedor) AS ref_fornecedor, COALESCE(NULLIF(v.mawb_mbl, ''), dfv.mawb_mbl) AS mawb_mbl, COALESCE((SELECT username FROM dados_dachser.t_users_dachser WHERE id = v.criado_por_user_id LIMIT 1), (SELECT user_name FROM dados_dachser.t_voucher_logs WHERE voucher_id = v.id AND acao IN ('VOUCHER_CRIADO', 'MASTER_CRIADO', 'VOUCHER_CRIADO_LOTE', 'VOUCHER_CRIADO_BATCH', 'IMPORTADO_RM', 'MASTER_CRIADO_LOTE', 'VOUCHER_MASTER_CRIADO', 'LOTE_FINALIZADO') ORDER BY data_hora ASC LIMIT 1)) AS criado_por_nome, COALESCE((SELECT username FROM dados_dachser.t_users_dachser WHERE id = v.criado_por_user_id LIMIT 1), (SELECT user_name FROM dados_dachser.t_voucher_logs WHERE voucher_id = v.id AND acao IN ('VOUCHER_CRIADO', 'MASTER_CRIADO', 'VOUCHER_CRIADO_LOTE', 'VOUCHER_CRIADO_BATCH', 'IMPORTADO_RM', 'MASTER_CRIADO_LOTE', 'VOUCHER_MASTER_CRIADO', 'LOTE_FINALIZADO') ORDER BY data_hora ASC LIMIT 1)) AS criado_por_user_name, (SELECT user_name FROM dados_dachser.t_voucher_logs WHERE voucher_id = v.id AND acao IN ('ENVIADO_OPERACAO','APROVADO_FISCAL','APROVADO_SUPERVISOR','REENVIO_APOS_AJUSTE','APROVADO_URGENTE','MASTER_APROVADO_OPERACAO','VOUCHER_ENVIADO','RASCUNHO_ENVIADO','VOUCHER_CRIADO','VOUCHER_CRIADO_LOTE','IMPORTADO_RM','MASTER_CRIADO','VOUCHER_CRIADO_BATCH') ORDER BY data_hora DESC LIMIT 1) AS enviado_por_nome, (SELECT user_name FROM dados_dachser.t_voucher_logs WHERE voucher_id = v.id AND acao IN ('ENVIADO_OPERACAO','APROVADO_FISCAL','APROVADO_SUPERVISOR','REENVIO_APOS_AJUSTE','APROVADO_URGENTE','MASTER_APROVADO_OPERACAO','VOUCHER_ENVIADO','RASCUNHO_ENVIADO','VOUCHER_CRIADO','VOUCHER_CRIADO_LOTE','IMPORTADO_RM','MASTER_CRIADO','VOUCHER_CRIADO_BATCH') ORDER BY data_hora DESC LIMIT 1) AS enviado_por_user_name FROM dados_dachser.t_vouchers v LEFT JOIN (SELECT nd, MIN(id_rm) as id_rm, MAX(created_by) as created_by, MAX(data_emissao) as data_emissao, MIN(numero_processo) as numero_processo, MAX(razao_social) as razao_social, MAX(nome_beneficiario) as nome_beneficiario, MAX(valor_nf) as valor_nf, MAX(ref_fornecedor) as ref_fornecedor, MAX(mawb_mbl) as mawb_mbl FROM dados_dachser.t_dados_financeiro_voucher WHERE SUBSTRING_INDEX(TRIM(nd), ' ', 1) COLLATE utf8mb4_unicode_ci = ? COLLATE utf8mb4_unicode_ci GROUP BY nd) dfv ON SUBSTRING_INDEX(TRIM(dfv.nd), ' ', 1) COLLATE utf8mb4_general_ci = SUBSTRING_INDEX(TRIM(v.numero_spo), ' ', 1) COLLATE utf8mb4_general_ci WHERE v.sync_status = 'ATIVO' AND (v.voucher_master_id IS NULL OR v.voucher_master_id = '') AND v.etapa_atual NOT IN ('AGUARDANDO_DOCUMENTOS_LOTE','CONSOLIDADO_NO_MASTER') AND (SUBSTRING_INDEX(TRIM(v.numero_spo), ' ', 1) COLLATE utf8mb4_unicode_ci = ? COLLATE utf8mb4_unicode_ci OR dfv.nd IS NOT NULL) ORDER BY v.updated_at DESC LIMIT 100", [$exactNd, $exactNd]) ?: [];
        }
...
$router->get('fin/vouchers/report', function ($params) {
    try {
        ...
        $vouchers = finQuery("SELECT v.*, dfv.ref_fornecedor as dfv_ref_fornecedor, dfv.mawb_mbl as dfv_mawb_mbl, COALESCE(NULLIF(v.ref_fornecedor, ''), dfv.ref_fornecedor) AS ref_fornecedor, COALESCE(NULLIF(v.mawb_mbl, ''), dfv.mawb_mbl) AS mawb_mbl, COALESCE(u_criado.username, (SELECT user_name FROM dados_dachser.t_voucher_logs WHERE voucher_id = v.id AND acao IN ('VOUCHER_CRIADO', 'MASTER_CRIADO', 'VOUCHER_CRIADO_LOTE', 'VOUCHER_CRIADO_BATCH', 'IMPORTADO_RM', 'MASTER_CRIADO_LOTE', 'VOUCHER_MASTER_CRIADO', 'LOTE_FINALIZADO') ORDER BY data_hora ASC LIMIT 1)) AS criado_por_nome, COALESCE(u_criado.username, (SELECT user_name FROM dados_dachser.t_voucher_logs WHERE voucher_id = v.id AND acao IN ('VOUCHER_CRIADO', 'MASTER_CRIADO', 'VOUCHER_CRIADO_LOTE', 'VOUCHER_CRIADO_BATCH', 'IMPORTADO_RM', 'MASTER_CRIADO_LOTE', 'VOUCHER_MASTER_CRIADO', 'LOTE_FINALIZADO') ORDER BY data_hora ASC LIMIT 1)) AS criado_por_username, (SELECT user_name FROM dados_dachser.t_voucher_logs WHERE voucher_id = v.id AND acao IN ('ENVIADO_OPERACAO','APROVADO_FISCAL','APROVADO_SUPERVISOR','REENVIO_APOS_AJUSTE','APROVADO_URGENTE','MASTER_APROVADO_OPERACAO','VOUCHER_ENVIADO','RASCUNHO_ENVIADO','VOUCHER_CRIADO','VOUCHER_CRIADO_LOTE','IMPORTADO_RM','MASTER_CRIADO','VOUCHER_CRIADO_BATCH') ORDER BY data_hora DESC LIMIT 1) AS enviado_por_nome, (SELECT user_name FROM dados_dachser.t_voucher_logs WHERE voucher_id = v.id AND acao IN ('ENVIADO_OPERACAO','APROVADO_FISCAL','APROVADO_SUPERVISOR','REENVIO_APOS_AJUSTE','APROVADO_URGENTE','MASTER_APROVADO_OPERACAO','VOUCHER_ENVIADO','RASCUNHO_ENVIADO','VOUCHER_CRIADO','VOUCHER_CRIADO_LOTE','IMPORTADO_RM','MASTER_CRIADO','VOUCHER_CRIADO_BATCH') ORDER BY data_hora DESC LIMIT 1) AS enviado_por_user_name FROM dados_dachser.t_vouchers v LEFT JOIN dados_dachser.t_users_dachser u_criado ON v.criado_por_user_id = u_criado.id LEFT JOIN (SELECT nd, MAX(created_by) AS created_by, MAX(ref_fornecedor) as ref_fornecedor, MAX(mawb_mbl) as mawb_mbl FROM dados_dachser.t_dados_financeiro_voucher GROUP BY nd) dfv ON SUBSTRING_INDEX(TRIM(dfv.nd), ' ', 1) COLLATE utf8mb4_general_ci = SUBSTRING_INDEX(TRIM(v.numero_spo), ' ', 1) COLLATE utf8mb4_general_ci $whereClause ORDER BY v.created_at DESC LIMIT 5000", $qp) ?: [];
        sendJson(['success' => true, 'vouchers' => $vouchers]);
    } catch (Exception $e) {
        sendJson(['success' => false, 'error' => $e->getMessage()], 500);
    }
...
$router->post('fin/vouchers', function ($params) {
    try {
        ...
        $criadoPorUserId = $nil($d['criado_por_user_id'] ?? $d['userId'] ?? $d['user_id'] ?? $d['created_by_user_id'] ?? null);
        finQuery(
