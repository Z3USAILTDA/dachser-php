...
Deno.serve(async (req) => {
  ...
  if (!searchValue) {
    return new Response(
      JSON.stringify({ success: false, error: 'Search value is required' }),
      { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
  ...
  try {
    ...
    switch (searchType) {
      case 'bl':
        queryParam = `transportDocumentReference=${encodeURIComponent(searchValue)}`;
        break;
      case 'container':
        queryParam = `equipmentReference=${encodeURIComponent(searchValue)}`;
        break;
      case 'booking':
      default:
        queryParam = `carrierBookingReference=${encodeURIComponent(searchValue)}`;
        break;
    }

    const apiUrl = `https://api.hlag.com/hlag/external/v2/events/?${queryParam}`;
    ...
    if (response.status === 204) {
      ...
      console.log('API returned 204 No Content - no data found');
      return new Response(
        JSON.stringify({ 
          success: false, 
    ...
    if (!responseText || responseText.trim() === '') {
      console.log('Empty response body from API');
      const searchTypeLabels = {
        booking: 'Booking',
        bl: 'Bill of Lading',
        container: 'Container',
    ...
    let bookingNumber = searchValue;
    if (searchType !== 'booking' && events.length > 0) {
      const firstEvent = events[0];
      bookingNumber = firstEvent.carrierBookingReference || searchValue;
    }
    ...
    for (const event of events) {
      ...
      if (event.transportDocumentReference && !bookingInfo.transportDocumentReference) {
        bookingInfo.transportDocumentReference = event.transportDocumentReference;
      }
