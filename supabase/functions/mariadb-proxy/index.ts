  ...
  try {
    ...
    switch (action) {
      ...
      case 'get_aging_by_client': {
        ...
        const clientLastUpdateResult = await client.query(`
          SELECT MAX(data_insert) as last_update FROM dados_dachser.t_dados_financeiro_nfs
        `);
        const clientLastUpdate = clientLastUpdateResult?.[0]?.last_update || null;
        
        console.log(`[get_aging_by_client] Found ${clientRows.length} clients`);
        ...
        const cnpjDetailSql = `
          ...
          LEFT JOIN dados_dachser.t_fin_cliente_grupo g
            ON g.razao_social COLLATE utf8mb4_unicode_ci
             = UPPER(TRIM(COALESCE(t.razao_social,''))) COLLATE utf8mb4_unicode_ci
          WHERE COALESCE(sd.active, 1) = 1
            AND COALESCE(g.grupo, TRIM(SUBSTRING_INDEX(COALESCE(t.razao_social, 'Sem Cliente'), '-', 1))) COLLATE utf8mb4_unicode_ci
      ...
      case 'get_cct_shipments': {
        ...
        // ==================== UNIFIED CTE QUERY ====================
        console.log('CCT: Executing unified CTE query...');
        const cctRows = await client.query(`
          WITH base_cct AS (
            SELECT
              h.id,
              h.hawb,
              h.hawb_normalizado,
              h.data_emissao,
              h.data_consulta_sucesso,
              h.attempts_used,
              h.consulted_at,
              h.response_http_status,
              h.request_url,
              h.response_hash,
              h.json_identificacao,
              h.json_conhecimento_carga_detalhada,
              h.json_partes_estoque,
              h.json_bloqueios_ativos,
              h.json_bloqueios_baixados,
              h.json_frete,
              h.json_manuseios_especiais,
              h.json_viagens_associadas,
              h.json_divergencias,
              h.json_mawb_awb_associados,
              h.json_itens_carga,
              h.json_contatos_consignatario,
              h.json_documentos_saida,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_identificacao, '$.situacaoLead')) AS situacao_lead,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_identificacao, '$.situacaoPortal')) AS situacao_portal,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_identificacao, '$.dataUltimaAtualizacaoCargaDetalhada')) AS data_ultima_atualizacao_carga,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_identificacao, '$.dataIntegracaoLead')) AS data_integracao_lead,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_conhecimento_carga_detalhada, '$.ruc')) AS ruc,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_conhecimento_carga_detalhada, '$.tipo')) AS tipo_documento_carga,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_conhecimento_carga_detalhada, '$.situacao')) AS situacao_carga_codigo,
              ...
              JSON_UNQUOTE(JSON_EXTRACT(h.json_mawb_awb_associados, '$[0].dataEmissao')) AS mawb_data_emissao,
              ...
              JSON_UNQUOTE(JSON_EXTRACT(h.json_viagens_associadas, '$[1].identificacaoViagem')) AS voo_2,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_viagens_associadas, '$[1].dataPartidaPrevista')) AS voo_2_data_partida,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_partes_estoque, '$[0].situacaoAtual')) AS situacao_estoque_atual,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_partes_estoque, '$[0].dataHoraSituacaoAtual')) AS data_hora_situacao_estoque,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_partes_estoque, '$[0].numeroDocumentoSaida')) AS numero_documento_saida,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_partes_estoque, '$[0].unidadeRfb')) AS unidade_rfb,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_partes_estoque, '$[0].cnpjResponsavelAtual')) AS cnpj_responsavel_atual,
              ...
              JSON_UNQUOTE(JSON_EXTRACT(h.json_documentos_saida, '$[0].numero')) AS duimp_numero,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_documentos_saida, '$[0].situacaoDuimp')) AS duimp_situacao,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_documentos_saida, '$[0].canalDuimp')) AS duimp_canal,
              JSON_UNQUOTE(JSON_EXTRACT(h.json_documentos_saida, '$[0].dataHoraVinculacao')) AS duimp_data_vinculacao,
              JSON_LENGTH(h.json_bloqueios_ativos) AS qtd_bloqueios_ativos,
              JSON_LENGTH(h.json_bloqueios_baixados) AS qtd_bloqueios_baixados,
              JSON_LENGTH(h.json_divergencias) AS qtd_divergencias,
              JSON_LENGTH(h.json_manuseios_especiais) AS qtd_tratamentos_especiais
          ...
          aereo_latest AS (
            ...
            FROM (
              SELECT
                ...
                a.data_emissao AS data_emissao_aereo,
                a.etd,
                a.eta,
                a.gross_weight_kg,
                a.volume_cbm,
                ...
                ROW_NUMBER() OVER (
                  ...
                  ORDER BY a.created_at DESC, a.data_emissao DESC
                ) AS rn
          ...
          SELECT
            ...
            c.hawb_normalizado,
            c.data_emissao,
            c.data_consulta_sucesso,
            c.consulted_at,
            c.response_http_status,
            c.attempts_used,
            COALESCE(a.consignee_nome, c.nome_consignatario_leadcomex) AS cliente,
            a.clerk AS analista,
            a.clerk_email AS analista_email,
            a.awb_number AS master,
            COALESCE(c.mawb_associado, c.mawb_associado_fallback, a.awb_number) AS master_final,
            c.aeroporto_origem,
            c.aeroporto_destino,
            CONCAT(COALESCE(c.aeroporto_origem, 'N/A'), ' -> ', COALESCE(c.aeroporto_destino, 'N/A')) AS rota,
            c.situacao_lead,
            c.situacao_portal,
            c.situacao_estoque_atual,
            CASE
              WHEN UPPER(COALESCE(c.situacao_estoque_atual, '')) = 'ENTREGUE' THEN 'Entregue'
              WHEN UPPER(COALESCE(c.situacao_estoque_atual, '')) = 'RECEPCIONADA' THEN 'Recepcionada'
              WHEN UPPER(COALESCE(c.situacao_estoque_atual, '')) = 'MANIFESTADA' THEN 'Manifestada'
              WHEN UPPER(COALESCE(c.situacao_portal, '')) = 'ENTREGUE' THEN 'Entregue'
              WHEN UPPER(COALESCE(c.situacao_portal, '')) = 'RECEPCIONADA' THEN 'Recepcionada'
              WHEN UPPER(COALESCE(c.situacao_portal, '')) = 'MANIFESTADA' THEN 'Manifestada'
              WHEN c.response_http_status = 200 THEN 'Em Trânsito Terrestre'
              ELSE 'AGUARDANDO_CONSULTA'
            END AS status_tela,
            CASE
              WHEN UPPER(COALESCE(c.situacao_estoque_atual, '')) = 'ENTREGUE' THEN 'Cumprido'
              WHEN UPPER(COALESCE(c.situacao_portal, '')) = 'ENTREGUE' THEN 'Cumprido'
              ELSE NULL
            END AS sla_status_sql,
            c.ruc,
            c.recinto_aduaneiro_destino,
            c.cnpj_consignatario,
            c.indicador_madeira,
            ...
            a.gross_weight_kg AS peso_recebido_aereo,
            a.volume_cbm AS volume_cbm_aereo,
            a.pieces AS pieces_aereo,
            a.etd,
            a.eta,
            c.voo_1 AS voo_principal,
            c.voo_1_data_partida AS data_decolagem,
            c.voo_1_data_partida AS data_decolagem_ultimo_trecho,
            c.qtd_bloqueios_ativos,
            c.qtd_bloqueios_baixados,
            c.qtd_divergencias,
            c.qtd_tratamentos_especiais,
            c.duimp_numero,
            c.duimp_situacao,
            c.duimp_canal,
            c.duimp_data_vinculacao,
            c.numero_documento_saida,
            c.unidade_rfb,
            c.cnpj_responsavel_atual,
            c.data_hora_situacao_estoque,
            c.data_ultima_atualizacao_carga,
        ...
        console.log(`CCT: Unified query returned ${(cctRows || []).length} rows`);

        if (!cctRows || cctRows.length === 0) {
          console.log('CCT: No valid HAWBs found, returning empty');
          ...
          if (stillMissing.length > 0) {
            for (let i = 0; i < stillMissing.length; i += 100) {
              ...
              const mdRows = await client.query(
                `SELECT hawb, nome_analista, email_analista FROM ${database}.t_master_dados WHERE hawb IN (${placeholders}) AND nome_analista IS NOT NULL AND TRIM(nome_analista) != '' ORDER BY data_insert DESC`,
                chunk
              );
        ...
        const enrichedShipments = (cctRows || []).map((row: any) => {
          ...
          // Parse json_frete for info_frete
          ...
          // Parse bloqueios for has_bloqueio
          const bloqueiosAtivos = safeParseJson(row.json_bloqueios_ativos) || [];
          const hasBloqueio = (row.qtd_bloqueios_ativos > 0) || (Array.isArray(bloqueiosAtivos) && bloqueiosAtivos.length > 0);

          // Manuseios as array for manuseios_especiais_rfb
          const manuseiosCodes = Array.isArray(manuseios)
            ? manuseios.map((m: any) => typeof m === 'string' ? m : m?.codigo || m?.code || '').filter(Boolean)
            : [];

          return {
            id: (row.hawb_normalizado || row.hawb || '').trim(),
            house: row.hawb || '',
            ...
            arr_datetime: null,
            ruc: row.ruc || null,
            recinto_aduaneiro: row.recinto_aduaneiro_destino || null,
            numero_voo: row.voo_principal || null,
            ...
            sla_status_sql: row.sla_status_sql || null,
          };
        });
        ...
        const processedShipments = (enrichedShipments || []).map((row: any) => {
          ...
          return {
            ...
            ultimo_evento_codigo: row.ultimo_evento_codigo || statusCode,
            ultimo_evento_descricao: row.ultimo_evento_descricao || statusCode,
      ...
      case 'get_voucher_responsaveis_emails': {
        ...
        // Fallback equivalente para creator_email: se o voucher não tem criado_por_user_id
        // (típico de vouchers vindos da sync), resolver pelo último log de envio/criação
        // feito por usuário humano da Operação.
        let creatorEmail: string | null = r.creator_email || null;
        let creatorUsername: string | null = r.creator_username || null;
        if (!creatorEmail) {
          try {
            const logRows = await client.query(`
              ...
              JOIN ai_agente.t_users_dachser u
                ON CAST(u.id AS CHAR) COLLATE utf8mb4_unicode_ci = CAST(l.user_id AS CHAR) COLLATE utf8mb4_unicode_ci
              WHERE l.voucher_id = ?
                AND l.acao IN ('VOUCHER_ENVIADO','RASCUNHO_ENVIADO','MASTER_APROVADO_OPERACAO','REENVIO_APOS_AJUSTE','VOUCHER_CRIADO','RASCUNHO_CRIADO')
                AND l.user_id IS NOT NULL
                AND l.user_id <> '0'
                AND l.user_id <> ''
                AND u.email IS NOT NULL
            ...
            `, [voucher_id]);
            if (logRows?.[0]?.email) {
              creatorEmail = String(logRows[0].email);
              creatorUsername = creatorUsername || String(logRows[0].username || '');
              console.log(`[get_voucher_responsaveis_emails] creator_email resolved via log fallback for voucher ${voucher_id}: ${creatorEmail}`);
            }
          } catch (logFallbackErr) {
            console.warn('[get_voucher_responsaveis_emails] creator log fallback failed:', logFallbackErr);
          }
        }
      ...
      case 'get_awb_tracking_events': {
        ...
        const FORCED_TIMELINES: Record<string, { events: any[]; tracking_failed: boolean }> = {
          ...
          '724-20906771': {
            ...
            events: [
              ...
              mkForcedEvent('724-20906771', 5, 'BKD', 'BKD - Booked on Flight LX-0093, GRU-ZRH', '2026-03-13T11:26:00', 'GRU', 1, '16.5 K'),
              mkForcedEvent('724-20906771', 6, 'BKD', 'BKD - Booked on Flight LX-0340, ZRH-LHR', '2026-03-10T13:42:00', 'ZRH', 1, '16.5 K'),
            ],
          },
          '724-07461451': {
            tracking_failed: false,
          ...
          '549-42692926': {
            tracking_failed: false,
            events: [
              mkForcedEvent('549-42692926', 1, 'ARR - Destino', 'ARR - Destino - Flight Arrived at BOG (L7 2538 VCP-BOG)', '2026-03-15T17:23:00', 'BOG', 1, '40.00 KGS'),
          ...
          '074-04758283': {
            tracking_failed: false,
            events: [
              mkForcedEvent('074-04758283', 1, 'BKD', '7 pieces booked at FRA', '2026-03-03T09:08:00', 'FRA', 7, null),
              mkForcedEvent('074-04758283', 2, 'FWB', 'Customer FWB processed (7 pieces)', '2026-03-04T16:29:00', 'FRA', 7, null),
              mkForcedEvent('074-04758283', 3, 'FOH', '7 pieces on hand at FRA', '2026-03-05T22:14:00', 'FRA', 7, null),
              mkForcedEvent('074-04758283', 4, 'RCS', '7 pieces received at FRA', '2026-03-06T16:40:00', 'FRA', 7, null),
              mkForcedEvent('074-04758283', 5, 'DEP', 'KL8356 (FRA→AMS) - 5 pieces departed from FRA', '2026-03-07T14:59:00', 'FRA', 5, null),
              mkForcedEvent('074-04758283', 6, 'DEP', 'KL8352 (FRA→AMS) - 2 pieces departed from FRA', '2026-03-07T17:29:00', 'FRA', 2, null),
              mkForcedEvent('074-04758283', 7, 'ARR', 'KL8356 (FRA→AMS) - 5 pieces arrived at AMS', '2026-03-07T21:55:00', 'AMS', 5, null),
              mkForcedEvent('074-04758283', 8, 'RCF', 'KL8356 (FRA→AMS) - 5 pieces received at AMS', '2026-03-07T22:33:00', 'AMS', 5, null),
              mkForcedEvent('074-04758283', 9, 'ARR', 'KL8352 (FRA→AMS) - 2 pieces arrived at AMS', '2026-03-08T00:36:00', 'AMS', 2, null),
              mkForcedEvent('074-04758283', 10, 'RCF', 'KL8352 (FRA→AMS) - 2 pieces received at AMS', '2026-03-08T07:54:00', 'AMS', 2, null),
              mkForcedEvent('074-04758283', 11, 'DEP', 'KL0791 (AMS→GRU) - 7 pieces departed from AMS', '2026-03-09T11:00:00', 'AMS', 7, null),
              mkForcedEvent('074-04758283', 12, 'ARR', 'KL0791 (AMS→GRU) - 7 pieces arrived at GRU', '2026-03-09T19:34:00', 'GRU', 7, null),
              mkForcedEvent('074-04758283', 13, 'RCF', 'KL0791 (AMS→GRU) - 7 pieces received at GRU', '2026-03-09T19:45:00', 'GRU', 7, null),
            ],
          },
          '057-56797090': {
            tracking_failed: false,
            events: [
              mkForcedEvent('057-56797090', 1, 'BKD', '1 piece booked at ZRH', '2026-03-06T18:11:00', 'ZRH', 1, null),
              mkForcedEvent('057-56797090', 2, 'FWB', 'Customer FWB processed (1 piece)', '2026-03-09T14:00:00', 'ZRH', 1, null),
              mkForcedEvent('057-56797090', 3, 'FOH', '1 piece on hand at ZRH', '2026-03-10T14:28:00', 'ZRH', 1, null),
              mkForcedEvent('057-56797090', 4, 'RCS', '1 piece received at ZRH', '2026-03-10T15:28:00', 'ZRH', 1, null),
              mkForcedEvent('057-56797090', 5, 'DEP', 'AF0501D (ZRH→CDG) - 1 piece departed from ZRH', '2026-03-11T20:27:00', 'ZRH', 1, null),
              mkForcedEvent('057-56797090', 6, 'ARR', 'AF0501D (ZRH→CDG) - 1 piece arrived at CDG', '2026-03-12T06:32:00', 'CDG', 1, null),
              mkForcedEvent('057-56797090', 7, 'RCF', 'AF0501D (ZRH→CDG) - 1 piece received at CDG', '2026-03-12T19:53:00', 'CDG', 1, null),
              mkForcedEvent('057-56797090', 8, 'DEP', 'AF0460 (CDG→GRU) - 1 piece departed from CDG', '2026-03-13T11:40:00', 'CDG', 1, null),
              mkForcedEvent('057-56797090', 9, 'ARR', 'AF0460 (CDG→GRU) - 1 piece arrived at GRU', '2026-03-13T19:08:00', 'GRU', 1, null),
              mkForcedEvent('057-56797090', 10, 'RCF', 'AF0460 (CDG→GRU) - 1 piece received at GRU', '2026-03-13T19:20:00', 'GRU', 1, null),
              mkForcedEvent('057-56797090', 11, 'NFD', '1 piece ready to be picked up at GRU', '2026-03-13T19:22:00', 'GRU', 1, null),
            ],
          },
          '057-03764530': {
            tracking_failed: false,
            events: [
              mkForcedEvent('057-03764530', 1, 'BKD', '1 piece booked at GIG', '2026-03-12T14:44:00', 'GIG', 1, null),
              mkForcedEvent('057-03764530', 2, 'FWB', 'Customer FWB processed (1 piece)', '2026-03-13T12:37:00', 'GIG', 1, null),
            ],
          },
          '057-03727710': {
            tracking_failed: false,
            events: [
              mkForcedEvent('057-03727710', 1, 'BKD', '1 piece booked at BCN', '2026-03-11T11:27:00', 'BCN', 1, null),
              mkForcedEvent('057-03727710', 2, 'FWB', 'Customer FWB processed (1 piece)', '2026-03-11T12:44:00', 'BCN', 1, null),
              mkForcedEvent('057-03727710', 3, 'FOH', '1 piece on hand at BCN', '2026-03-13T10:54:00', 'BCN', 1, null),
              mkForcedEvent('057-03727710', 4, 'RCS', '1 piece received at BCN', '2026-03-13T18:07:00', 'BCN', 1, null),
              mkForcedEvent('057-03727710', 5, 'DEP', 'AF0677D - 1 piece departed from BCN (TRANSPORTE TERRESTRE)', '2026-03-14T00:01:00', 'BCN', 1, null),
            ],
          },
          '057-03659810': {
            tracking_failed: false,
            events: [
              mkForcedEvent('057-03659810', 1, 'BKD', '2 pieces booked at OPO', '2026-03-05T14:58:00', 'OPO', 2, null),
              mkForcedEvent('057-03659810', 2, 'FWB', 'Customer FWB processed (2 pieces)', '2026-03-05T17:45:00', 'OPO', 2, null),
              mkForcedEvent('057-03659810', 3, 'FOH', '2 pieces on hand at OPO', '2026-03-06T17:08:00', 'OPO', 2, null),
              mkForcedEvent('057-03659810', 4, 'RCS', '2 pieces received at OPO', '2026-03-06T22:28:00', 'OPO', 2, null),
              mkForcedEvent('057-03659810', 5, 'DEP', 'AF0947D (OPO→CDG) - 2 pieces departed from OPO', '2026-03-07T01:25:00', 'OPO', 2, null),
              mkForcedEvent('057-03659810', 6, 'ARR', 'AF0947D (OPO→CDG) - 2 pieces arrived at CDG', '2026-03-08T21:25:00', 'CDG', 2, null),
              mkForcedEvent('057-03659810', 7, 'RCF', 'AF0947D (OPO→CDG) - 2 pieces received at CDG', '2026-03-09T11:18:00', 'CDG', 2, null),
              mkForcedEvent('057-03659810', 8, 'DEP', 'AF0460 (CDG→GRU) - 2 pieces departed from CDG', '2026-03-13T11:40:00', 'CDG', 2, null),
              mkForcedEvent('057-03659810', 9, 'ARR', 'AF0460 (CDG→GRU) - 2 pieces arrived at GRU', '2026-03-13T19:08:00', 'GRU', 2, null),
              mkForcedEvent('057-03659810', 10, 'RCF', 'AF0460 (CDG→GRU) - 2 pieces received at GRU', '2026-03-13T19:19:00', 'GRU', 2, null),
              mkForcedEvent('057-03659810', 11, 'NFD', '2 pieces ready to be picked up at GRU', '2026-03-13T19:21:00', 'GRU', 2, null),
            ],
          },
          '045-90418226': {
            tracking_failed: false,
            events: [
              mkForcedEvent('045-90418226', 1, 'BKD', 'Booking Confirmed LA 8113 MEX-GRU - 1 / 4.70KGS', '2026-03-06T10:18:00', 'MEX', 1, '4.70KGS'),
              mkForcedEvent('045-90418226', 2, 'RCS', 'Shipment Received at MEX - 1 / 4.70KGS', '2026-03-09T15:07:00', 'MEX', 1, '4.70KGS'),
              mkForcedEvent('045-90418226', 3, 'FOH', 'Freight on Hand at MEX - 1 / 4.70KGS', '2026-03-09T15:07:00', 'MEX', 1, '4.70KGS'),
              mkForcedEvent('045-90418226', 4, 'DEP', 'LA 8113 (MEX→GRU) - Flight Departed from MEX', '2026-03-10T17:28:00', 'MEX', 0, '0.00KGS'),
              mkForcedEvent('045-90418226', 5, 'ARR', 'LA 8113 (MEX→GRU) - Flight Arrived at GRU - 1 / 4.70KGS', '2026-03-11T05:12:00', 'GRU', 1, '4.70KGS'),
              mkForcedEvent('045-90418226', 6, 'RCF', 'LA 8113 (MEX→GRU) - Received from Flight at GRU - 1 / 4.70KGS', '2026-03-11T12:02:00', 'GRU', 1, '4.70KGS'),
              mkForcedEvent('045-90418226', 7, 'NFD', 'Agent Notified at GRU - 1 / 4.71KGS', '2026-03-11T12:02:00', 'GRU', 1, '4.71KGS'),
            ],
          },
          '045-90418215': {
            tracking_failed: false,
            events: [
              mkForcedEvent('045-90418215', 1, 'BKD', 'Booking Confirmed LA 8113 MEX-GRU - 2 / 411.00KGS', '2026-03-06T15:41:00', 'MEX', 2, '411.00KGS'),
              mkForcedEvent('045-90418215', 2, 'DEP', 'LA 8113 (MEX→GRU) - Flight Departed from MEX - 2 / 411.00KGS', '2026-03-10T17:24:00', 'MEX', 2, '411.00KGS'),
              mkForcedEvent('045-90418215', 3, 'ARR', 'LA 8113 (MEX→GRU) - Flight Arrived at GRU - 2 / 411.00KGS', '2026-03-11T05:12:00', 'GRU', 2, '411.00KGS'),
              mkForcedEvent('045-90418215', 4, 'DEP', 'LA 8113 (MEX→GRU) - Flight Departed from MEX - 2 / 411.00KGS', '2026-03-11T08:51:00', 'MEX', 2, '411.00KGS'),
              mkForcedEvent('045-90418215', 5, 'RCF', 'LA 8113 (MEX→GRU) - Received from Flight at GRU - 2 / 411.00KGS', '2026-03-11T12:02:00', 'GRU', 2, '411.00KGS'),
              mkForcedEvent('045-90418215', 6, 'BKD', 'Booking Confirmed M3 6559 GRU-VCP - 2 / 411.00KGS', '2026-03-11T15:30:00', 'GRU', 2, '411.00KGS'),
            ],
          },
          '045-21167753': {
            tracking_failed: false,
            events: [
              mkForcedEvent('045-21167753', 1, 'FOH', 'Freight on Hand at FRA - 10 / 1047.90KGS', '2026-03-06T15:41:00', 'FRA', 10, '1047.90KGS'),
              mkForcedEvent('045-21167753', 2, 'RCS', 'Shipment Received at FRA - 10 / 1047.90KGS', '2026-03-06T16:55:00', 'FRA', 10, '1047.90KGS'),
              mkForcedEvent('045-21167753', 3, 'DIS', 'DIS at FRA - 10 / 1047.90KGS', '2026-03-06T16:55:00', 'FRA', 10, '1047.90KGS'),
              mkForcedEvent('045-21167753', 4, 'DEP', 'LA 5201-T (FRA→MAD) - 10 / 1047.90KGS', '2026-03-09T16:34:00', 'FRA', 10, '1047.90KGS'),
              mkForcedEvent('045-21167753', 5, 'RCF', 'LA 5201-T - Received at MAD - 10 / 1048.00KGS', '2026-03-10T02:43:00', 'MAD', 10, '1048.00KGS'),
              mkForcedEvent('045-21167753', 6, 'MAN', 'LA 8065 (MAD→GRU) - Manifested at MAD - 10 / 1048.00KGS', '2026-03-10T19:11:00', 'MAD', 10, '1048.00KGS'),
              mkForcedEvent('045-21167753', 7, 'BKD', 'Booking Confirmed LA 8065 MAD-GRU - 10 / 1047.90KGS', '2026-03-10T19:44:00', 'MAD', 10, '1047.90KGS'),
              mkForcedEvent('045-21167753', 8, 'DEP', 'LA 8065 (MAD→GRU) - Departed from MAD - 10 / 1048.00KGS', '2026-03-10T23:08:00', 'MAD', 10, '1048.00KGS'),
              mkForcedEvent('045-21167753', 9, 'DEP', 'LA 8065 (MAD→GRU) - Departed from MAD - 10 / 1048.00KGS', '2026-03-11T02:33:00', 'MAD', 10, '1048.00KGS'),
              mkForcedEvent('045-21167753', 10, 'ARR', 'LA 8065 (MAD→GRU) - Arrived at GRU - 10 / 1048.00KGS', '2026-03-11T05:09:00', 'GRU', 10, '1048.00KGS'),
              mkForcedEvent('045-21167753', 11, 'RCF', 'LA 8065 (MAD→GRU) - Received at GRU - 10 / 1048.00KGS', '2026-03-11T11:57:00', 'GRU', 10, '1048.00KGS'),
              mkForcedEvent('045-21167753', 12, 'NFD', 'Agent Notified at GRU - 10 / 1047.91KGS', '2026-03-11T11:59:00', 'GRU', 10, '1047.91KGS'),
            ],
          },
          '045-21167720': {
            tracking_failed: false,
            events: [
              mkForcedEvent('045-21167720', 1, 'BKD', 'Booking Confirmed LA 5491-T FRA - 99 / 6600.00KGS', '2026-03-10T11:22:00', 'FRA', 99, '6600.00KGS'),
              mkForcedEvent('045-21167720', 2, 'BKD', 'Booking Confirmed LA 8147 LIS-GRU - 56 / 6170.50KGS', '2026-03-11T07:10:00', 'LIS', 56, '6170.50KGS'),
              mkForcedEvent('045-21167720', 3, 'DEP', 'LA 5491-T (FRA→LIS) - 56 / 6170.50KGS (TRANSPORTE TERRESTRE)', '2026-03-11T16:52:00', 'FRA', 56, '6170.50KGS'),
              mkForcedEvent('045-21167720', 4, 'BKD', 'Booking Confirmed LA 8147 LIS-GRU - 56 / 6170.50KGS', '2026-03-13T13:57:00', 'LIS', 56, '6170.50KGS'),
              mkForcedEvent('045-21167720', 5, 'RCF', 'LA 5491-T - Received at LIS - 56 / 6170.50KGS', '2026-03-13T14:35:00', 'LIS', 56, '6170.50KGS'),
              mkForcedEvent('045-21167720', 6, 'DEP', 'LA 5491-T (FRA→LIS) - 56 / 6170.50KGS (TRANSPORTE TERRESTRE)', '2026-03-13T14:57:00', 'FRA', 56, '6170.50KGS'),
            ],
          },
          '045-12829191': {
            tracking_failed: false,
            events: [
              mkForcedEvent('045-12829191', 1, 'BKD', 'Booking Confirmed LA 8147 LIS-GRU - 1 / 693.00KGS', '2026-03-13T12:51:00', 'LIS', 1, '693.00KGS'),
              mkForcedEvent('045-12829191', 2, 'BKD', 'Booking Confirmed LA 5463-T FRA - 1 / 693.00KGS (TRANSPORTE TERRESTRE)', '2026-03-13T14:24:00', 'FRA', 1, '693.00KGS'),
            ],
          },
          '016-45294826': {
            tracking_failed: false,
            events: [
              mkForcedEvent('016-45294826', 1, 'BKD', 'Booking confirmed at DFW', '2026-03-09T17:40:00', 'DFW', null, null),
              mkForcedEvent('016-45294826', 2, 'RCS', 'Received from Shipper at DFW', '2026-03-11T14:37:00', 'DFW', null, null),
              mkForcedEvent('016-45294826', 3, 'MAN', 'Manifested at DFW', '2026-03-11T14:37:00', 'DFW', null, null),
              mkForcedEvent('016-45294826', 4, 'DEP', 'Departed from DFW', '2026-03-11T20:27:00', 'DFW', null, null),
              mkForcedEvent('016-45294826', 5, 'ARR', 'Arrived at ORD', '2026-03-11T23:10:00', 'ORD', null, null),
              mkForcedEvent('016-45294826', 6, 'RCF', 'Received from Flight at ORD', '2026-03-12T00:49:00', 'ORD', null, null),
              mkForcedEvent('016-45294826', 7, 'BKD', 'Booking confirmed at ORD', '2026-03-12T15:23:00', 'ORD', null, null),
              mkForcedEvent('016-45294826', 8, 'MAN', 'Manifested at ORD', '2026-03-13T10:47:00', 'ORD', null, null),
              mkForcedEvent('016-45294826', 9, 'DEP', 'Departed from ORD', '2026-03-13T19:36:00', 'ORD', null, null),
              mkForcedEvent('016-45294826', 10, 'ARR', 'Arrived at GRU', '2026-03-14T08:05:00', 'GRU', null, null),
        ...
        try {
          ...
          const extractPiecesFromDesc = (text: string): number | null => {
            ...
            // Ignore booking/reservation events: pieces shown reflect flight capacity, not actual cargo
            if (/(^|[^A-Z])(BOOKED|BOOKING)([^A-Z]|$)/i.test(text)) {
              return null;
            }
      ...
      case 'insert_dados_rm': {
        ...
        await client.execute(`
          ...
          VALUES (?, ?, 0, ?, ?, ?, ?, ?, ?, ?)
        `, [finalIdRm, numeroSpoRm || null, voucherBoletoFinal, chavePixFinal, pixTipoChave || null, formaPag || null, fornecedorRm || null, regrasFormaPagFinal, tipoExec || 'A_DEFINIR']);
      ...
      case 'create_voucher_master': {
        ...
        if (missingProcessos.length > 0) {
          const financialRows = await client.query(`
            SELECT nd, razao_social, cnpj, valor_nf, moeda, data_vencimento, modal, id_rm, created_by
            FROM dados_dachser.t_dados_financeiro_voucher
        ...
        if (!mblContainers || mblContainers.length === 0) {
          ...
          try {
            const histDiscovery = await queryWithRetry(() => client.query(
              ...
              [mbl]
            ), { label: 'demurrage_history_discovery', attempts: 3 });

            if (histDiscovery && histDiscovery.length > 0) {
              console.log(`[demurrage_get_containers_by_mbl] Found ${histDiscovery.length} containers in history`);
              mblContainers = [];
              ...
              for (const row of histDiscovery) {
                ...
                try {
                  const histRows = await queryWithRetry(() => client.query(
                    ...
                    [numero, numero, numero]
                  ), { label: 'demurrage_hist_dates_3b', attempts: 2 });
                ...
                } catch (e) { console.error(`History dates error for ${numero}:`, e); }

                const ftStartedAt = dischargeDate ? `${dischargeDate} 00:00:00` : null;

                mblContainers.push({
                  id: `hist-${numero}`,
                  numero,
                  mbl: (row.mbl_id || mbl).trim(),
                  booking: null,
                  cliente: null,
                  armador: null,
                  tipo_processo: null,
            ...
            console.error('[demurrage_get_containers_by_mbl] History discovery fallback error:', e);
          }
        }

        // Step 4: Final fallback - synthetic record from pre-invoice metadata
        if ((!mblContainers || mblContainers.length === 0) && invoice_number) {
          console.log(`[demurrage_get_containers_by_mbl] Step 4: Creating synthetic record from pre-invoice: ${invoice_number}`);
          try {
            const piRows = await queryWithRetry(() => client.query(
              `SELECT client_name, vessel_name, voyage_number, origin_port, destination_port, total_usd, issue_date
              FROM dados_dachser.t_dachser_demurrage_pre_invoices
              WHERE invoice_number = ?
              LIMIT 1`,
              [invoice_number]
            ), { label: 'demurrage_synthetic_fallback', attempts: 2 });
            ...
            if (piRows && piRows.length > 0) {
              ...
              mblContainers = [{
                ...
                mbl: mbl,
                booking: null,
                cliente: pi.client_name || null,
                armador: null,
                tipo_processo: null,
                porto_origem: pi.origin_port || null,
                porto_destino: pi.destination_port || null,
                navio: pi.vessel_name || null,
                vessel_imo: null,
                voyage: pi.voyage_number || null,
                etd: null,
                eta: null,
                last_event: null,
                container_status: null,
            ...
            console.error('[demurrage_get_containers_by_mbl] Synthetic fallback error:', e);
          }
        }
        ...
        for (const rm of (newVouchers || [])) {
          try {
            ...
            `, [
              ...
              rm.valor_nf || 0,
              rm.moeda || 'BRL',
              toDateSafe(rm.data_vencimento) || new Date().toISOString().split('T')[0],
              toDateSafe(rm.data_emissao),
              rm.nome_cobranca === 'CLIENTE' ? 'CLIENTE' : 'DACHSER',
              mapFormaPag(rm.forma_pag),
              rm.id_rm,
              rm.data_insert,
        ...
        if (action === 'create_voucher_batch_import') {
          ...
          for (const it of items) {
            ...
            if (it.status === 'VALID') {
              ...
              const etapaDestino = urgenciaTipo === 'URGENTE_REAL'
                ? 'SUPERVISOR'
