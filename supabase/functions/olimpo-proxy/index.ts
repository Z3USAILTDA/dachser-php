  ...
  try {
    ...
    if (action === 'hapag_batch_discover') {
      ...
      try {
        ...
        // Hapag office codes to strip from MBL for API variations
        ...
        const getMblVariations = (mblId: string): string[] => {
          ...
          // Also try without the 4-char prefix office code (e.g. HLCUBKK260145016 → BKK260145016 is booking ref)
          if (upper.startsWith('HLCU') && upper.length > 10) {
            // Try the raw number portion as transportDocumentReference
        ...
        for (const row of pendingRows) {
          ...
          for (const variation of variations) {
            ...
            try {
              ...
              // Extract unique containers with tracking data
              const containerMap = new Map<string, any[]>();
              ...
              for (const [ctrNo, ctrEvents] of containerMap.entries()) {
                ...
                // Latest equipment event for status
                const latestEquip = sorted.find((e: any) => e.eventType === 'EQUIPMENT');
                if (latestEquip) {
                  const code = latestEquip.equipmentEventTypeCode || '';
                  const statusMap: Record<string, string> = { 'LOAD': 'LOADED', 'DISC': 'DISCHARGED', 'GTIN': 'GATE_IN', 'GTOT': 'GATE_OUT', 'PICK': 'PICKED_UP', 'DROP': 'DROPPED_OFF' };
              ...
              discovered++;
              found = true;
              break; // Success, no need to try more variations
            } catch (fetchErr: any) {
              clearTimeout(timeoutId);
