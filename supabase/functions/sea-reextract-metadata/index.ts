...
serve(async (req) => {
  ...
  try {
    ...
    // Connect to MariaDB
    const dbHost = (Deno.env.get('MARIADB_SEA_HOST') || Deno.env.get('MARIADB_OPS_HOST')) ?? '';
    const dbPort = parseInt((Deno.env.get('MARIADB_SEA_PORT') || Deno.env.get('MARIADB_OPS_PORT')) ?? '3306');
