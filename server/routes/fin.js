    ...
    if (exactNd) {
      vouchers = await finQuery(`
        SELECT v.*,
          ...
          (SELECT user_name FROM dados_dachser.t_voucher_logs WHERE voucher_id = v.id AND acao IN ('ENVIADO_OPERACAO','APROVADO_FISCAL','APROVADO_SUPERVISOR','REENVIO_APOS_AJUSTE','APROVADO_URGENTE','MASTER_APROVADO_OPERACAO','VOUCHER_ENVIADO','RASCUNHO_ENVIADO','VOUCHER_CRIADO','VOUCHER_CRIADO_LOTE','IMPORTADO_RM','MASTER_CRIADO','VOUCHER_CRIADO_BATCH') ORDER BY data_hora DESC LIMIT 1) AS enviado_por_nome,
