  ...
  app.get('/api/olimpo/cobranca/client-detail', async (req, res) => {
    try {
      ...
      if (cnpjs.length) {
        ...
        try {
          contatos = await query(`
              SELECT REPLACE(REPLACE(REPLACE(cnpj,'.',''),'/',''),'-','') AS cnpjClean,
                     MAX(nome_contato) AS nome_contato,
                     LOWER(TRIM(email_contato)) AS email_contato
                FROM ${DB}.t_dados_financeiro_contatos
               WHERE REPLACE(REPLACE(REPLACE(cnpj,'.',''),'/',''),'-','') IN (${ph})
                 AND email_contato IS NOT NULL AND email_contato <> ''
                 AND ativo = 1
                 AND (funcao IS NULL OR funcao IN (
                       'Cobrança Zeus','ZEUS AI','ZEUS ASO','ZEUS DEX','ZEUS DIM','ZEUS SI','ZEUS TCK'
                     ))
               GROUP BY cnpjClean, LOWER(TRIM(email_contato))
               ORDER BY cnpjClean, email_contato
            `, cnpjs);
        } catch {}
      }
  ...
  app.get('/api/sea/tracking/cleanup-orphans', async (_req, res) => {
    try {
      const [result] = await getPoolFor('olimpo').query(`
        ...
        INNER JOIN (
          SELECT DISTINCT mbl_id FROM ${DB}.t_sea_tracking_current
          WHERE active = 1 AND container IS NOT NULL
            AND UPPER(container) NOT IN ('PENDENTE','NAO_ENCONTRADO')
        ) sub ON sub.mbl_id = ts.mbl_id
