...
import { UploadZone } from "@/components/sea/UploadZone";
import { FileItem } from "@/components/sea/FileItem";
...
export default function SubmeterHblMbl() {
  ...
  const handleCopyResult = () => {
    ...
    try {
      ...
      const textToCopy = analysisResult.result_text.replace(/```json[\s\S]*?```/g, '') // Remove markdown JSON blocks
      .replace(/\{"hbl_shipping_data"[\s\S]*?\}\s*$/g, '') // Remove raw JSON at end
      .replace(/\{"document_metadata"[\s\S]*?\}\s*$/g, '') // Remove document_metadata JSON
      .replace(/^\s*\{"document_metadata"\s*:\s*\{[^}]*\}\s*\}\s*$/gm, '') // Remove inline metadata
