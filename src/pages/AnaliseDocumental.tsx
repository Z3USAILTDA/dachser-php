...
const AnaliseDocumental = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      data = data.filter(
        (item) =>
          item.pdf_name?.toLowerCase().includes(q) ||
          item.excel_name?.toLowerCase().includes(q) ||
  ...
  const pendingCount = useMemo(
    () => processes.filter((p) => p.status === "pendente").length,
    [processes]
  );
    ...
    <PageLayout title="DACHSER" subtitle="Análise Documental" pageIcon={FileSearch}>
      ...
      <PageCard>
        <FilterBar
          ...
          searchPlaceholder="Buscar por arquivo ou usuário"
          filters={[
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-[0.85rem] text-[#aaaaaa]">
            {searchTerm || statusFilter !== "todos"
              ...
              : "Nenhuma comparação realizada ainda. Clique em \"Novo Processo\" para começar."}
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedData.map((item) => (
                    <tr
                      key={item.id}
                      className="border-b border-[rgba(255,255,255,.06)] hover:bg-[rgba(255,255,255,.03)] transition-colors"
                    >
                      ...
                      <td className="px-[10px] py-[9px]">
                        <div className="flex justify-end gap-1.5">
                          ...
                          <button
                            onClick={() => handleDeleteClick(item.id, item.pdf_name)}
