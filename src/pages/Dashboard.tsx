  ...
  return <div className="min-h-screen relative overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-4 md:px-6 py-3 bg-background/30 backdrop-blur-sm border-b border-border/30">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleLogout} className="w-9 h-9 rounded-full border border-border/50 flex items-center justify-center bg-background/70 text-primary hover:bg-background hover:shadow-[0_0_12px_hsl(var(--primary)/0.6)] transition-all duration-200" title="Sair">
            <LogOut size={18} />
          </button>
        </div>
      </header>
      ...
      <main className="relative z-10 pt-24 pb-20 px-4">
        ...
        <div className="relative w-full max-w-5xl mx-auto h-28 mb-8 hidden md:block">
          {[...Array(6)].map((_, i) => <div key={i} className="absolute top-0 left-1/2 w-0.5 h-full rounded-full opacity-70" style={{
        ...
        <div className="flex justify-center items-start gap-2 px-4">
          {filteredMenuItems.map(item => <div key={item.id} className="flex flex-col items-center" style={{
          width: '170px'
        }}>
              {/* Card */}
              <div onClick={() => item.href ? navigate(item.href) : toggleMenu(item.id)} className={`
                ...
                `} style={{
            background: activeMenu === item.id ? 'linear-gradient(135deg, #ffc800, #ffe680)' : 'rgba(4, 10, 30, 0.75)',
            boxShadow: activeMenu === item.id ? '0 22px 60px rgba(0, 0, 0, 0.85), 0 0 22px rgba(255, 200, 0, 0.65)' : '0 22px 60px rgba(0, 0, 0, 0.85), 0 0 0 1px rgba(255, 255, 255, 0.03)',
                      ...
                      border: '1px solid rgba(255, 255, 255, 0.08)'
                          ...
                          right: '90px'
                                    ...
                                    {child.subChildren.map((subChild, subIdx) => <div key={subIdx} className="relative flex flex-col items-center pt-0 w-[180px]">
                                        ...
                                        <button onClick={() => navigate(subChild.href)} className="mt-2 min-w-[180px] px-5 py-2.5 rounded-full text-foreground text-sm font-medium hover:-translate-y-0.5 transition-all duration-200" style={{
                            background: 'rgba(4, 10, 30, 0.75)',
                            boxShadow: '0 12px 30px rgba(0, 0, 0, 0.6), 0 0 0 1px rgba(255, 255, 255, 0.08)',
                            backdropFilter: 'blur(18px)',
                            border: '1px solid rgba(255, 255, 255, 0.08)'
                          ...
                          </div> : <button onClick={() => child.href && navigate(child.href)} className="mt-2 min-w-[180px] px-5 py-2.5 rounded-full text-foreground text-sm font-medium hover:-translate-y-0.5 transition-all duration-200" style={{
                  background: 'rgba(4, 10, 30, 0.75)',
                  boxShadow: '0 12px 30px rgba(0, 0, 0, 0.6), 0 0 0 1px rgba(255, 255, 255, 0.08)',
                  backdropFilter: 'blur(18px)',
                  border: '1px solid rgba(255, 255, 255, 0.08)'
