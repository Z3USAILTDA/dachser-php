...
export default function CCTDashboard() {
  ...
  return <PageLayout title="DACHSER" subtitle="CRONOS CCT — Monitoramento de Carga Aérea" showBack={true} pageIcon={LayoutDashboard} headerActions={<div className="flex items-center gap-2.5">
      ...
      <div className="space-y-6">
        {error && <div className="rounded-xl bg-rose-500/10 border border-rose-500/30 p-3 flex items-center gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          {metricFilter && <span className="px-3 py-1 rounded-full bg-[rgba(255,200,0,0.15)] text-[#ffc800] border border-[#ffc800]/40 text-[0.75rem] font-mono cursor-pointer hover:bg-[rgba(255,200,0,0.25)] transition" onClick={() => setMetricFilter(null)}>
              Filtro ativo: {metricFilter === "total" ? "Todos" : metricFilter === "alerta" ? "Em Alerta" : metricFilter === "critico" ? "Críticos" : "Eventos 24h"} ✕
            </span>}
          {metrics.emTransito > 0 && !metricFilter && <span className="px-3 py-1 rounded-full bg-blue-500/15 text-blue-400 border border-blue-500/40 text-[0.75rem] font-mono">
              {metrics.emTransito} aguardando manifestação
            </span>}
        </div>

        {/* Unified Table */}
        {isLoading ? <div className="h-96 rounded-2xl bg-[rgba(5,6,18,0.9)] border border-[rgba(255,255,255,0.12)] animate-pulse" /> : <ProcessosTable processos={processos} onAssignAnalista={handleOpenAssignDialog} metricFilter={metricFilter} />}
      </div>
