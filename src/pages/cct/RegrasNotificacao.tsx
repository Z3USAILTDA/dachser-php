...
export default function RegrasNotificacao() {
  ...
  const filteredRegras = regras.filter(r => {
    ...
    return (
      ...
      r.aeroportos.some(a => a.toLowerCase().includes(term))
    );
  });

  const handleEdit = (regra: CCTRegraNotificacao) => {
  ...
  return (
    <PageLayout title="DACHSER" subtitle="Regras de Notificação — Sistema Hermes">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Buscar por cliente, CNPJ ou aeroporto..."
        ...
        <Button onClick={handleNew} className="bg-amber-500 hover:bg-amber-600 text-black">
              ...
              filteredRegras.map((regra) => (
                <TableRow key={regra.id} className="border-white/8 hover:bg-white/5">
                  ...
                  <TableCell className="text-white/70 font-mono text-xs">
                    {regra.cnpj_consignatario || '—'}
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {regra.aeroportos.length > 0 ? (
                        regra.aeroportos.slice(0, 5).map(a => (
                          <Badge key={a} variant="secondary" className="bg-amber-500/20 text-amber-300 text-[10px]">
                            {a}
                          </Badge>
                        ))
                      ) : (
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {regra.canais.map(c => (
                        <Badge key={c} variant="secondary" className={`text-[10px] ${getCanalColor(c)}`}>
                          {c.replace(/_/g, ' ')}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <TableCell className="text-white/50 text-xs">
                    {regra.created_at ? format(new Date(regra.created_at), 'dd/MM/yyyy') : '—'}
                  </TableCell>
