...
export default function ConsoleTecnico() {
  ...
  const triggerSync = async (serviceName: string) => {
    ...
    setSyncStatuses(prev => prev.map(s => 
      s.name === serviceName ? { ...s, status: 'syncing' } : s
    ));
  ...
  useEffect(() => {
    checkSyncStatus();
    fetchUsers();
    addLog('info', 'Sistema', 'Console técnico inicializado');
  }, [checkSyncStatus, fetchUsers, addLog]);
              ...
              <Card key={sync.name} className="bg-[rgba(5,6,18,0.9)] border-white/12">
                ...
                <CardContent className="space-y-3">
                  <Badge variant="outline" className={getStatusColor(sync.status)}>
                    {sync.status === 'online' ? 'Conectado' : 
                     sync.status === 'offline' ? 'Desconectado' :
                     sync.status === 'error' ? 'Erro' : 'Sincronizando...'}
                  </Badge>
                  
                  {sync.lastSync && (
                    <p className="text-xs text-white/50 flex items-center gap-1">
                      <Clock className="h-3 w-3" />
                      Última sync: {format(new Date(sync.lastSync), 'dd/MM HH:mm:ss')}
                    </p>
                  )}

                  {sync.recordCount !== undefined && (
                    <p className="text-xs text-white/50">
                      {sync.recordCount} registros
                    </p>
                  )}

                  {sync.errorMessage && (
                    <p className="text-xs text-red-400">{sync.errorMessage}</p>
                  )}

                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => triggerSync(sync.name)}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            <Card className="bg-[rgba(5,6,18,0.9)] border-white/12">
              <CardContent className="pt-4">
                ...
                <p className="text-xs text-white/50">Processos Ativos</p>
              </CardContent>
            </Card>
            <Card className="bg-[rgba(5,6,18,0.9)] border-white/12">
              <CardContent className="pt-4">
                <p className="text-2xl font-bold text-amber-400">{users.length}</p>
                <p className="text-xs text-white/50">Analistas</p>
              </CardContent>
            </Card>
            <Card className="bg-[rgba(5,6,18,0.9)] border-white/12">
              <CardContent className="pt-4">
            ...
            <Card className="bg-[rgba(5,6,18,0.9)] border-white/12">
              <CardContent className="pt-4">
                ...
                <p className="text-xs text-white/50">Logs (sessão)</p>
              </CardContent>
            </Card>
          </div>
        </TabsContent>
            ...
            <ScrollArea className="h-[500px]">
              <div className="p-4 font-mono text-xs space-y-1">
                {filteredLogs.length === 0 ? (
                  <p className="text-white/40 text-center py-8">Nenhum log encontrado</p>
                ) : (
          ...
          <Card className="bg-[rgba(5,6,18,0.9)] border-white/12">
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between py-3 border-b border-white/8">
                <div>
                  <p className="text-white font-medium">Sync Automático (MariaDB)</p>
                  <p className="text-xs text-white/50">Sincronizar dados a cada 5 minutos</p>
                </div>
              ...
              <div className="flex items-center justify-between py-3 border-b border-white/8">
                <div>
                  ...
                  <p className="text-xs text-white/50">Enviar e-mails para exceções críticas</p>
                </div>
                <Switch defaultChecked />
              </div>
              <div className="flex items-center justify-between py-3 border-b border-white/8">
                <div>
                  <p className="text-white font-medium">Logs Detalhados</p>
                  <p className="text-xs text-white/50">Registrar todas as operações no console</p>
                </div>
                <Switch />
              </div>
              <div className="flex items-center justify-between py-3">
                <div>
                  ...
                  <p className="text-xs text-white/50">Desativar todas as sincronizações</p>
                </div>
                <Switch />
              </div>
            </CardContent>
          ...
          <Card className="bg-[rgba(5,6,18,0.9)] border-white/12">
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white font-mono">MariaDB 10.6</span>
              </div>
              <div className="flex justify-between">
                <span className="text-white/50">Edge Functions</span>
