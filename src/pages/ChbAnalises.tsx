...
export default function ChbAnalises() {
  ...
  return <div className="min-h-screen relative overflow-x-hidden">
      ...
      <div className="relative z-10 max-w-[95%] mx-auto px-2 pt-5 pb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-[18px]">
          ...
          <header>
            ...
            <p className="text-[0.9rem] text-[#aaaaaa] mt-0.5">Desembaraço — Esteira de Análises (CHB)</p>
      ...
      <main className="relative z-10 max-w-[95%] mx-auto mb-12 px-2 space-y-[18px]">
        ...
        boxShadow: '0 18px 40px rgba(0,0,0,.85)'
          ...
          <div className="space-y-3">
            <div className="relative">
              ...
              <input type="text" placeholder="Buscar por referência ou consignee" value={search} onChange={e => setSearch(e.target.value)} className="h-9 w-full pl-10 pr-4 rounded-full border border-[rgba(255,255,255,.14)] bg-[#13141a] text-[#f5f5f5] text-[0.78rem] placeholder:text-[#666] focus:outline-none focus:border-[#ffc800] focus:shadow-[0_0_0_1px_rgba(255,200,0,.8)]" />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 justify-between">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button onClick={handleRefresh} disabled={isRefreshing} className="inline-flex items-center gap-1.5 h-8 px-3 rounded-full bg-[rgba(255,255,255,.05)] border border-[rgba(255,255,255,.25)] text-[#f5f5f5] text-[0.78rem] font-semibold hover:bg-[rgba(255,255,255,.08)] disabled:opacity-50">
        ...
        {/* CARD DA TABELA */}
        <section className="rounded-2xl p-[14px_16px_12px]" style={{
        ...
        boxShadow: '0 18px 40px rgba(0,0,0,.9)'
          ...
          {filteredItems.length === 0 ? <div className="mt-4 p-3 rounded-xl border border-[rgba(255,255,255,.25)] bg-[rgba(255,255,255,.06)] text-[0.85rem] text-[#aaaaaa]">
      ...
      {/* History Modal */}
      <Dialog open={historyModal.open} onOpenChange={open => setHistoryModal({
      ...historyModal,
      open
    })}>
        <DialogContent className="max-w-[900px] max-h-[80vh] overflow-auto" style={{
        background: '#111',
        ...
        borderRadius: '12px'
          ...
          <div className="mt-4 space-y-3">
            {historyModal.history.length === 0 ? <div className="text-[#aaaaaa] text-sm">Sem histórico aprovado.</div> : historyModal.history.map(entry => <div key={entry.id} className="rounded-xl p-3" style={{
            background: '#0e0e0e',
            border: '1px solid rgba(255,255,255,.1)'
                  ...
                  {entry.result_html ? <div className="rounded-lg p-3 max-h-[200px] overflow-auto text-sm" style={{
              background: '#0f0f0f',
              border: '1px solid rgba(255,255,255,.14)'
            }} dangerouslySetInnerHTML={{
              __html: entry.result_html
            }} /> : entry.result_text ? <pre className="rounded-lg p-3 max-h-[200px] overflow-auto text-sm whitespace-pre-wrap" style={{
              background: '#0f0f0f',
              border: '1px solid rgba(255,255,255,.14)'
            }}>
                      {entry.result_text}
                    </pre> : <div className="text-[#aaaaaa] text-sm">—</div>}
                  
                  <div className="mt-2 flex gap-2">
                    <button onClick={() => handleCopyResult(entry.result_text || entry.result_html)} className="h-8 px-3 rounded-xl flex items-center gap-1.5 bg-[rgba(255,255,255,.06)] border border-[rgba(255,255,255,.16)] text-[#f5f5f5] text-[0.8rem] font-bold hover:border-[#ffc800] hover:text-[#ffc800] hover:bg-[rgba(255,255,255,.08)] transition-all">
                      <Copy size={14} />
                      Copiar
                    </button>
                  </div>
      ...
      open
        ...
        border: '1px solid rgba(255,255,255,.14)'
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => deleteDialog.itemId && handleDelete(deleteDialog.itemId)} className="rounded-xl bg-[#ff4d4f] hover:bg-[#ff6b6d] border-0">
              Remover
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
