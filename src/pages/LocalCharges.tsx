...
export default function LocalCharges() {
  ...
  const pageSize = 20;
  const [hapagData, setHapagData] = useState<CompanyData>({
    rows: [],
    meta: {
      updated_at: null,
      effective: null
  ...
  const [mscData, setMscData] = useState<CompanyData>({
    rows: [],
    meta: {
      updated_at: null,
      effective: null
  ...
  const [cmaData, setCmaData] = useState<CompanyData>({
    rows: [],
    meta: {
      updated_at: null,
      effective: null
  ...
  const [hmmData, setHmmData] = useState<CompanyData>({
    rows: [],
    meta: {
      updated_at: null,
      effective: null
  ...
  const [oneData, setOneData] = useState<CompanyData>({
    rows: [],
    meta: {
      updated_at: null,
      effective: null
  ...
  const filteredRows = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(row => Object.values(row).some(val => String(val).toLowerCase().includes(q)));
    }
    return filtered;
  ...
  const rightContent = <div className="flex items-center gap-2">
      ...
      <Button onClick={fetchLocalCharges} disabled={isLoading} variant="outline" className="h-8 rounded-full text-xs">
        <RefreshCw className={`h-4 w-4 mr-2 ${isLoading ? 'animate-spin' : ''}`} />
        Atualizar
      </Button>
      
    </div>;
  return <PageLayout title="DACHSER" subtitle="Local Charges – Tabela Consolidada" rightContent={rightContent} pageIcon={Receipt} backTo="/dashboard">
      <PageCard>
        {/* Header with filters */}
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative w-full lg:w-64">
              ...
              <Input placeholder="Buscar por qualquer coluna..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9 h-9 text-sm rounded-full bg-[#13141a] border-border/50" />
            </div>
          </div>
        </div>
        ...
        <div className="border border-border/30 rounded-xl overflow-hidden">
          <Table>
            <TableHeader>
              ...
              }].map(col => <TableHead key={col.key} onClick={() => handleSort(col.key)} className={`text-[0.7rem] uppercase tracking-wider cursor-pointer hover:text-primary whitespace-nowrap ${col.align === 'right' ? 'text-right' : ''} ${sortColumn === col.key ? 'text-primary' : ''}`}>
                    {col.label}
            ...
            <TableBody>
              {isLoading ? <TableRow>
                ...
                </TableRow> : paginatedRows.length === 0 ? <TableRow>
                  <TableCell colSpan={10} className="text-center py-8 text-muted-foreground">
                    Nenhum registro encontrado
                  </TableCell>
                </TableRow> : paginatedRows.map((row, idx) => <TableRow key={row.id || idx} className="text-sm hover:bg-white/5">
                    <TableCell className="whitespace-nowrap">
                      <Badge variant="outline" className={`text-[0.68rem] ${armadorColors[row.empresa] || 'border-border/50'}`}>
                        {row.empresa || '-'}
                      </Badge>
                    </TableCell>
