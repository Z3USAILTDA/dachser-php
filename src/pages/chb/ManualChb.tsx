...
import { BookOpen, LayoutDashboard, FileCheck, AlertTriangle, Upload, CheckCircle2, HelpCircle, BookText, ChevronRight, Search, Ship, FileText, Eye } from "lucide-react";
...
const glossaryItems = [{
  term: 'HBL',
  definition: 'House Bill of Lading - Conhecimento de embarque emitido pelo agente consolidador.'
}, {
  term: 'MBL',
  definition: 'Master Bill of Lading - Conhecimento principal emitido pelo armador.'
}, {
  term: 'BL',
  definition: 'Bill of Lading - Documento de transporte marítimo que comprova o embarque.'
}, {
  term: 'Consignee',
  definition: 'Consignatário - Destinatário da mercadoria indicado no conhecimento.'
}, {
  term: 'Shipper',
  definition: 'Embarcador - Remetente da mercadoria.'
}, {
  term: 'Container',
  definition: 'Unidade de carga padronizada para transporte de mercadorias.'
}, {
  term: 'TEU',
  definition: 'Twenty-foot Equivalent Unit - Unidade de medida de capacidade de containers.'
}, {
  term: 'FCL',
  definition: 'Full Container Load - Container completo de um único embarcador.'
}, {
  term: 'LCL',
  definition: 'Less than Container Load - Carga consolidada de múltiplos embarcadores.'
}, {
  term: 'Invoice',
  definition: 'Fatura comercial com descrição e valor das mercadorias.'
}];
export default function ManualChb() {
  ...
  const filteredFaq = faqItems.filter(item => item.q.toLowerCase().includes(searchTerm.toLowerCase()) || item.a.toLowerCase().includes(searchTerm.toLowerCase()));
  const filteredGlossary = glossaryItems.filter(item => item.term.toLowerCase().includes(searchTerm.toLowerCase()) || item.definition.toLowerCase().includes(searchTerm.toLowerCase()));
  return <PageLayout title="DACHSER" subtitle="Manual do Usuário — Conferência CHB v1.0" backTo="/chb/conferences" pageIcon={BookOpen}>
      <div className="flex gap-6 items-start">
        {/* Sidebar Navigation */}
        <div ref={sidebarPlaceholderRef} className="w-64 shrink-0">
          <div ref={sidebarRef} className={cn("w-64 transition-all duration-200", isSticky ? "fixed top-6 z-40" : "relative")}>
            <Card className="bg-[rgba(5,6,18,0.9)] border-white/12 max-h-[calc(100vh-4rem)] overflow-y-auto">
              ...
              <CardContent className="p-0">
                <nav className="space-y-1 pb-4">
                  {sections.map(section => <button key={section.id} onClick={() => scrollToSection(section.id)} className={cn("w-full flex items-center gap-2 px-4 py-2 text-sm transition-colors", activeSection === section.id ? "bg-amber-500/20 text-amber-300 border-l-2 border-amber-400" : "text-white/60 hover:text-white hover:bg-white/5")}>
                      {section.icon}
                      {section.title}
        ...
        <main className="flex-1 space-y-8">
          ...
          <div className="relative">
            ...
            <Input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Buscar no manual..." className="pl-9 bg-white/5 border-white/12 text-white" />
          </div>
            ...
            <Card className="bg-[rgba(5,6,18,0.9)] border-white/12">
              ...
              <CardContent className="space-y-4 text-white/80">
                ...
                <div className="space-y-2">
                  {['1. Pré-Alerta - Recepção inicial e validação de documentos básicos', '2. Instrução - Conferência de dados e envio de instruções ao despachante', '3. DI/Fechamento - Validação final, registro da DI e conclusão do processo'].map((etapa, i) => <div key={etapa} className="flex items-center gap-3 p-2 rounded bg-white/5">
          ...
          <section ref={el => sectionRefs.current['faq'] = el} id="faq">
            <Card className="bg-[rgba(5,6,18,0.9)] border-white/12">
              ...
              <CardContent className="space-y-4">
                {(searchTerm ? filteredFaq : faqItems).map((item, i) => <div key={i} className="p-4 rounded bg-white/5 border border-white/10">
                    <h4 className="text-white font-medium mb-2">{item.q}</h4>
                    <p className="text-sm text-white/70">{item.a}</p>
                  </div>)}
                {searchTerm && filteredFaq.length === 0 && <p className="text-white/50 text-center py-4">Nenhuma pergunta encontrada para "{searchTerm}"</p>}
              </CardContent>
            </Card>
          </section>
          ...
          <section ref={el => sectionRefs.current['glossario'] = el} id="glossario">
            <Card className="bg-[rgba(5,6,18,0.9)] border-white/12">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {(searchTerm ? filteredGlossary : glossaryItems).map((item, i) => <div key={i} className="p-3 rounded bg-white/5 border border-white/10">
                  ...
                  {searchTerm && filteredGlossary.length === 0 && <p className="text-white/50 text-center py-4 col-span-2">Nenhum termo encontrado para "{searchTerm}"</p>}
                </div>
              </CardContent>
            </Card>
          </section>
