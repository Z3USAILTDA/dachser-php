            ...
            <ZeusChartCard title="Top Clientes" subtitle="Último mês" headerRight={<ExpandButton chartId="top-clientes" />}>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={topClientesData.map(d => ({ ...d, cliente: d.clienteShort }))} layout="vertical" margin={{ top: 5, right: 60, left: 10, bottom: 5 }}>
                  ...
                  <XAxis type="number" tick={AXIS_TICK} tickFormatter={(v) => formatCurrencyCompact(v)} tickLine={false} domain={[0, 'auto']} />
            ...
            <ZeusChartCard title="Qtd. por Modal — Último Mês" subtitle="Último mês" headerRight={<ExpandButton chartId="modal-ultimo" />}>
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={lastMonthModalData} layout="vertical" margin={{ top: 5, right: 60, left: 10, bottom: 5 }}>
                  <CartesianGrid {...GRID_PROPS} horizontal={false} /><XAxis type="number" tick={AXIS_TICK} tickLine={false} domain={[0, 'auto']} /><YAxis type="category" dataKey="name" tick={{ fill: '#94a3b8', fontSize: 11 }} width={60} tickLine={false} />
