...
const getTrackingUrl = (airlineCode: string, fullAwb: string): string | null => {
  ...
  const urlBuilders: Record<string, (iata: string, awb: string) => string> = {
    "001": (iata, awb) => `https://www.aacargo.com/mobile/tracking-details.html?awb=${iata}${awb}`,
    ...
    "075": (iata, awb) => `https://www.iagcargo.com/iagcargo/portlet/en/html/601/main/search?frame=true&awb.cia=${iata}&awb.cod=${awb}`,
    ...
    "873": (iata, awb) => `https://aerounion-icargo.ibsplc.aero/icargoauportal/portal/trackshipments?trkTxnValue=${iata}${awb}`,
  };
...
const Index = () => {
  ...
  // Base pré-filtrada para métricas dos cards — aplica as mesmas regras da tabela
  // mas SEM filtros de busca/companhia/analista (métricas são sempre globais).
  // Base para os cards: aplica os mesmos filtros de pré-processamento E filtros de usuário
  // ativos (airline, analyst, service, processType), mas SEM cardFilter.
  // Garante que os números dos cards coincidam exatamente com o que a tabela mostra.
  const metricsBase = React.useMemo(() => {
    const allowedStatuses = [
      "BKD","BKF","AWB","RCS","MAN","DEP","FOH","TFD","RCT","RCP","PRE","LOF",
      "ARRT","TDE","ARR","ARR - DESTINO","ARR - CONEXAO","ARR - CONEXÃO","RCF",
      "DIS","OFLD","NIL","NIF",
      "ERRO","COMPANY_NOT_REGISTERED","AWB_INVALID",
      "FFM","AUD","NFD","AWD","AWR","UNK","DLV",
    ];
    ...
    let awbs = statusAereoData.filter((awb) => {
      // Filtros de usuário (mesmos da tabela)
      ...
      if (awb.tipo_processo) {
        ...
        if (destino && destino !== 'N/A') isImport = brazilianAirports.includes(destino);
      }
      ...
      if (!matchesAirline || !matchesAnalyst || !matchesService || !matchesProcessType) return false;

      // Filtros de pré-processamento (retenção ARR/DLV e status permitidos)
      ...
      if ((lastEventCode === "ARR" || lastEventCode.startsWith("ARR - ")) && !hasAlert) {
        ...
        if (arrDatetime) {
          const hoursElapsed = (Date.now() - new Date(arrDatetime).getTime()) / (1000 * 60 * 60);
          if (hoursElapsed >= ARR_RETENTION_HOURS) return false;
        }
      }

      if (lastEventCode === "DLV" || statusToCheck === "DLV") {
        const dlvDate = awb.last_check ? new Date(awb.last_check).getTime() : null;
        if (dlvDate && (Date.now() - dlvDate) / (1000 * 60 * 60 * 24) >= 5) return false;
      }

      if (awb.tracking_failed === true) return true;
      return isAllowed;
    });
    ...
    return awbs;
  }, [statusAereoData, filterAirline, filterAnalyst, filterService, filterProcessType]);
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden">
      ...
      <main className="relative z-10 max-w-[95%] mx-auto mb-12 px-2 space-y-[18px]">
        ...
        <DashboardCards
          totalMonitorados={
            metricsBase.filter((awb) => {
              const s = (awb.status || "").toUpperCase();
              return s !== "COMPANY_NOT_REGISTERED" && s !== "ERRO" && s !== "DLV";
            }).length
          }
          emTransito={
            metricsBase.filter((awb) => {
