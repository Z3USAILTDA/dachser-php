...
export const DatabaseStatsPanel = ({
  ...
  return <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm" className="gap-2 bg-[rgba(0,0,0,.70)] backdrop-blur-sm border-[rgba(255,255,255,.18)] hover:bg-[rgba(0,0,0,.85)] hover:border-[#ffc800]/50 text-[#aaaaaa] hover:text-[#f5f5f5]" disabled={isLoading}>
          {isLoading ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Database className="h-4 w-4 text-[#ffc800]" />}
          <span className="hidden sm:inline text-xs">Base de Dados</span>
          <ChevronDown className="h-3 w-3 opacity-50" />
        </Button>
      </PopoverTrigger>
      
      <PopoverContent className="w-80 p-0 bg-[rgba(5,6,18,.95)] border-[rgba(255,255,255,.12)] backdrop-blur-xl" align="end">
        <div className="p-4 border-b border-[rgba(255,255,255,.08)]">
          
          
          {stats && <div className="mt-3 space-y-2 text-sm">
        ...
        {stats && stats.airlineBreakdown.length > 0 && <div className="p-3">
            ...
            <ScrollArea className="h-48">
              ...
              return <div key={airline.code} className="text-xs">
                      ...
                      <div className="h-1 bg-[rgba(255,255,255,.08)] rounded-full overflow-hidden">
                        <div className="h-full bg-[#ffc800]/60 rounded-full transition-all" style={{
                    width: `${percentage}%`
                  }} />
                      </div>
                    </div>;
