import { Database, ChevronDown, RefreshCw, Ship } from "lucide-react";
...
export interface SeaDbStats {
  lastUpdate: string | null;
  totalRecords: number;
  shippingLineBreakdown: ShippingLineBreakdown[];
}

interface SeaDbStatsPanelProps {
  stats: SeaDbStats | null;
  isLoading: boolean;
  onRefresh?: () => void;
}

export const SeaDbStatsPanel = ({ stats, isLoading, onRefresh }: SeaDbStatsPanelProps) => {
      ...
      >
        <div className="p-4 border-b border-[rgba(255,255,255,.08)]">
          <div className="flex items-center gap-2 mb-2">
            <Ship className="h-4 w-4 text-[#ffc800]" />
            <span className="text-sm font-medium text-[#f5f5f5]">Sea Export - Master Dados</span>
          </div>
          ...
          {stats && (
            <div className="mt-3 space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-[#888]">Total MBLs:</span>
        ...
        {stats && stats.shippingLineBreakdown.length > 0 && (
          <div className="p-3">
            <div className="flex items-center gap-2 mb-2">
              <Ship className="h-3.5 w-3.5 text-[#888]" />
              <span className="text-xs font-medium text-[#888]">Distribuição por Armador</span>
            </div>
            
            <ScrollArea className="h-48">
              <div className="space-y-1.5 pr-3">
                {stats.shippingLineBreakdown.map((line) => {
                  const percentage = ((line.count / stats.totalRecords) * 100).toFixed(1);
                  return (
                    <div key={line.code} className="text-xs">
                      <div className="flex items-center justify-between mb-0.5">
                        <span className="truncate flex-1">
                          <span className="font-mono text-[#ffc800]">{line.code}</span>
                          {line.name !== line.code && (
                            <span className="text-[#888] ml-1.5">{line.name}</span>
                          )}
                        </span>
...
export default SeaDbStatsPanel;
