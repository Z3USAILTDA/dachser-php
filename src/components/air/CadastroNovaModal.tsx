    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-[rgba(5,6,18,.97)] border-[rgba(255,255,255,.12)] text-white">
        ...
        <div className="space-y-5">
          ...
          {/* Troca de Master */}
          <div className="border-t border-[rgba(255,255,255,.1)] pt-5">
            <div className="rounded-xl border border-[rgba(255,255,255,.1)] bg-[rgba(255,255,255,.03)] p-4 space-y-4">
              <h3 className="text-sm font-semibold text-[#ffc800] flex items-center gap-2">
                <RefreshCw className="h-4 w-4" /> Troca de Master (Manifesto)
              </h3>
              <p className="text-xs text-[#888]">
                Faça upload de um PDF de manifesto DACHSER para trocar o MAWB de processos existentes.
              </p>
              ...
              <div
                ...
                className="border-2 border-dashed border-[rgba(255,255,255,.15)] rounded-xl p-5 text-center hover:border-[rgba(255,255,255,.3)] transition-colors cursor-pointer"
                onClick={() => {
                  const input = document.createElement("input");
                  input.type = "file";
                  input.accept = ".pdf";
              ...
              >
                {isExtractingSwap ? (
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-[#aaa]">Extraindo dados do manifesto...</p>
                  </div>
                ) : (
                  <div className="flex flex-col items-center gap-2">
                    <Upload className="h-6 w-6 text-[#ffc800]/60" />
                    <p className="text-sm text-[#aaa]">
              ...
              {swapHawbs.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="text-xs">Novo MAWB: {swapMawb}</Badge>
                      <Badge variant="secondary" className="text-xs">{swapHawbs.length} HAWB(s)</Badge>
                    </div>
                    <Button variant="ghost" size="sm" onClick={resetSwap} className="text-xs">Limpar</Button>
                  </div>

                  <div className="rounded-lg border border-[rgba(255,255,255,.1)] overflow-hidden">
                    <Table>
                      <TableHeader>
                        <TableRow>
                      ...
                      <TableBody>
                        {swapHawbs.map((h, i) => (
                          <TableRow key={i}>
                            ...
                            <TableCell className="font-mono text-xs text-[#ffc800]">{swapMawb}</TableCell>
                            <TableCell>
                              {swapResult ? (
                  ...
                  {swapResult && (
                    <div className="flex gap-3 text-xs">
                      {swapResult.updated.length > 0 && <span className="text-emerald-400">✓ {swapResult.updated.length} atualizado(s)</span>}
                      {swapResult.notFound.length > 0 && <span className="text-red-400">⚠ {swapResult.notFound.length} não encontrado(s): {swapResult.notFound.join(", ")}</span>}
                    </div>
                  )}

                  {!swapResult && (
                    <div className="flex justify-end">
                      <Button onClick={handleConfirmSwap} disabled={isSwapping || !swapMawb} className="gap-2 bg-[#ffc800] text-black hover:bg-[#e6b400]">
